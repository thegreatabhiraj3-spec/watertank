...
setInterval(()=>{
  if(tank.pump==='FILL'){tank.level_pct=Math.min(tank.level_pct+0.8,100);tank.volume_L=+(tank.level_pct/100*tank.tank_L).toFixed(1);if(tank.level_pct>=tank.high_pct){tank.pump='OFF';broadcast({type:'AUTOSTOP',reason:'high_level',level_pct:tank.level_pct});broadcast({type:'PUMP',state:'OFF'});}if(tank.fillTarget!==null&&tank.level_pct>=tank.fillTarget){const t=tank.fillTarget;tank.pump='OFF';tank.fillTarget=null;broadcast({type:'FILLTO_DONE',target_pct:t,actual_pct:tank.level_pct});broadcast({type:'PUMP',state:'OFF'});}}
  if(tank.pump==='DRAIN'){tank.level_pct=Math.max(tank.level_pct-0.8,0);tank.volume_L=+(tank.level_pct/100*tank.tank_L).toFixed(1);if(tank.level_pct<=tank.low_pct){tank.pump='OFF';broadcast({type:'AUTOSTOP',reason:'low_level',level_pct:tank.level_pct});broadcast({type:'PUMP',state:'OFF'});}}
...
wss.on('connection',ws=>{
  ...
  ws.on('message',raw=>{
    ...
    console.log('CMD:',cmd);
    if(cmd==='FILL'){tank.pump='FILL';broadcast({type:'PUMP',state:'FILL'});}
    else if(cmd==='DRAIN'){tank.pump='DRAIN';broadcast({type:'PUMP',state:'DRAIN'});}
    else if(cmd==='STOP'){tank.pump='OFF';tank.fillTarget=null;broadcast({type:'PUMP',state:'OFF'});}
    else if(cmd.startsWith('FILLTO ')){const p=parseFloat(cmd.split(' ')[1]);if(!isNaN(p)){tank.fillTarget=p;tank.pump='FILL';broadcast({type:'PUMP',state:'FILL'});}}
    ...
    else if(cmd==='SIMULATELEAK'){tank.leak=true;broadcast({type:'LEAKALERT',rate_pct_per_min:6.3});}
    else if(cmd==='CLEARLEAK'){tank.leak=false;broadcast({type:'LEAKCLEAR'});}
    else if(cmd==='NEXTDAY'){tank.dailyUsage.shift();tank.dailyUsage.push(+(20+Math.random()*60).toFixed(1));broadcast({type:'DAYDONE'});}
  });
  ws.on('close',()=>console.log('Client disconnected'));
});
httpServer.listen(PORT,'0.0.0.0',()=>{console.log('\n✓ Simulator running on port '+PORT+'\n  Fill/drain buttons work live\n  Tank starts at 58%\n  Updates every second\n');});
